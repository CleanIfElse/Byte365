	...
	<body>
				...
				<div class="dashboard">
					...
					<table width="100%" cellspacing="0" class="highlight" cellpadding="0">
						...
						<tbody id="fileBrowser">
							<tr ng-repeat="file in files" id="file-{{ file.UploadID }}">
					               <td class="file first">{{ file.FileName }}</td>
					               <td class="file">{{ file.FileSize }}</td>
					               <td class="file">{{ file.FileType }}</td>
							   <td class="file"> {{ file.UploadTime }}</td>
								<td class="file last"><div style="float: right;">
									<a class="dropdown-button btn" dropdown data-hover="true" href="javascript:void(0);" ng-data-activates="dropdown{{ file.UploadID }}">Options</a></div>
									<ul id="dropdown{{ file.UploadID }}" class="dropdown-content">
										<li><a href="#!" ng-onclick="downloadFile('{{ file.ShareKey }}', '{{ file.EncryptionKey }}')">Download</a></li>
										<li><a href="#!" ng-onclick="showShareLink('https://byte365.net/share/?page&k={{ file.ShareKey }}')">Share</a></li>
									</ul>
						   		</td>
					            </tr>
						</tbody>
		...
		<script>
           function fileController($scope,$http) {
              var url = "https://byte365.net/api/?getMyFiles";
